...
impl Point {
    ...
    pub fn get_interpolated_color(&self, buffer:&ImageBuffer) -> Option<Dn> {
        ...
        let x_cl = if self.x_cl() < buffer.width { self.x_cl() } else { self.x_fl() };
        let y_cl = if self.y_cl() < buffer.height { self.y_cl() } else { self.y_fl() };
    
        if x_cl >= buffer.width || y_cl >= buffer.height || self.x_fl() >= buffer.width || self.y_fl() >= buffer.height {
            return None;
        }
