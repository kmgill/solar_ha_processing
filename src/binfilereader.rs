...
impl BinFileReader {
    ...
    pub fn new_as_endiness(file_path:&String, endiness:Endian) -> BinFileReader {
        ...
        let map: Mmap = unsafe { 
            Mmap::map(&file_ptr).expect("Error creating memory map")
        };
    ...
    pub fn read_string(&self, start:usize, len:usize) -> String {
        let v:Vec<u8> = self.map[start..(start + len)].iter().map(|x| x.clone()).collect();
        String::from_utf8(v).expect("Failed reading string value")
    }

    fn read_bytes_raw_16byte(&self, start:usize) -> [u8; 16] {
        self.map[start..(start + 16)].try_into().expect("slice with incorrect length")
    }

    fn read_bytes_raw_8byte(&self, start:usize) -> [u8; 8] {
        self.map[start..(start + 8)].try_into().expect("slice with incorrect length")
    }

    fn read_bytes_raw_4byte(&self, start:usize) -> [u8; 4] {
        self.map[start..(start + 4)].try_into().expect("slice with incorrect length")
    }

    fn read_bytes_raw_2byte(&self, start:usize) -> [u8; 2] {
        self.map[start..(start + 2)].try_into().expect("slice with incorrect length")
    }

    fn read_bytes_raw_1byte(&self, start:usize) -> [u8; 1] {
        self.map[start..(start + 1)].try_into().expect("slice with incorrect length")
    }
    ...
    pub fn read_bytes(&self, start:usize, length:usize) -> Vec<u8> {
        self.map[start..(start + length)].iter().map(|x| x.clone()).collect()
    }
