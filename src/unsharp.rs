...
pub fn unsharp_mask_nbands(buffers:&Vec<ImageBuffer>, sigma:f32, amount:f32) -> error::Result<Vec<ImageBuffer>> {
    ...
    match guassian_blur_nband(&buffers, sigma) {
        Ok(blurred) => {
            ...
            for b in 0..blurred.len() {
                out_buffers.push( buffers[b].add(&buffers[b].subtract(&blurred[b]).unwrap().scale(amount).unwrap()).unwrap() );
