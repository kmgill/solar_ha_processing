...
impl RunnableSubcommand for ExtractFrame {
    fn run(&self) {
        ...
        if self.frame >= ser_file.frame_count {
            eprintln!("Error: Requested frame {} exceeds available frames {}", (self.frame + 1), ser_file.frame_count);
            process::exit(5);
        }

        let frame = ser_file.get_frame(self.frame).expect("Failed extracting frame");
