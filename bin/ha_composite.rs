...
fn main() {
    let matches = App::new(crate_name!())
                    ...
                    .arg(Arg::with_name(constants::param::PARAM_OUTPUT)
                        ...
                        .required(true)
                        .takes_value(true))
                    .arg(Arg::with_name(constants::param::PARAM_SCALAR)
                        .short(constants::param::PARAM_SCALAR_SHORT)
                        .long(constants::param::PARAM_SCALAR)
                        .value_name("SCALAR")
                        .help("Scalar to apply to 2nd image")
                        .required(false)
                        .takes_value(true))
    ...
    let scalar = match matches.is_present(constants::param::PARAM_SCALAR) {
        true => {
            let s = matches.value_of(constants::param::PARAM_SCALAR).unwrap();
            if util::string_is_valid_f32(&s) {
                s.parse::<f32>().unwrap()
            } else {
                eprintln!("Error: Invalid number specified for scalar");
                process::exit(1);
            }
        },
        false => 1.0
